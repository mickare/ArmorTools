  ...
  protected ModifyAction parseAction(final Player player, final int area) {
    ...
    if (area > 0) {
      ...
      return ModifyAction.click(a -> {
        a.eject();
        a.setPassenger(player);
        ArmorUtil.setMarker(a, false);
        a.setGravity(true);
        StepManager.getInstance().putMove(player, new FunMoveStepAction(Sets.newHashSet(a)));
  ...
  protected Callback<ArmorStand> doAreaAction(Player player, ModifyAction action) {
    ...
    if (action.isArea()) {
      ...
      StepManager.getInstance().putMove(player,
          new FunMoveStepAction(Sets.newHashSet(armorstands)));
